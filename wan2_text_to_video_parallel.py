...
def setup_cfg_parallel(pipe, model_id: str,
    ...
    state = {"call_idx": 0, "neg_embeds": None, "neg_kwargs": None, "pending": None}

    def _to(v, dev):
        return v.to(dev) if isinstance(v, torch.Tensor) else v

    def _run(transformer, dev, hs, ts, ehs, kw, out, errors, idx):
        try:
            out[idx] = transformer(
                hidden_states=_to(hs, dev),
                timestep=_to(ts, dev),
                encoder_hidden_states=_to(ehs, dev),
                **{k: _to(v, dev) for k, v in kw.items()},
            )
        ...
        if call_idx == 0:
            ...
            if state["neg_embeds"] is not None:
                ...
                t0 = threading.Thread(target=_run, args=(
                    transformer0, device0,
                    hidden_states, timestep, encoder_hidden_states, kwargs,
                    results, errors, 0))
                t1 = threading.Thread(target=_run, args=(
                    transformer1, device1,
                    hidden_states, timestep, state["neg_embeds"], state["neg_kwargs"],
                    results, errors, 1))
                t0.start(); t1.start()
                t0.join(); t1.join()
